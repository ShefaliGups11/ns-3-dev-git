...
//                   MakeEnumChecker (CLOSED, "Closed"))
    ...
    .AddAttribute ("UseEcn", "True to use ECN functionality",
                   BooleanValue (false),
                   MakeBooleanAccessor (&TcpSocketBase::m_useEcn),
                   MakeBooleanChecker ())
    .AddTraceSource ("RTO",
...
TcpSocketBase::ProcessSynSent (Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
  ...
  if (tcpflags == 0)
    ...
    { // Received SYN, move to SYN_RCVD state and respond with SYN+ACK
       ...
       */
      if (m_useEcn && (tcpflags & (TcpHeader::CWR | TcpHeader::ECE)) == (TcpHeader::CWR | TcpHeader::ECE))
        {
    ...
    { // Handshake completed
       ...
       */
      if (m_useEcn && (tcpflags & (TcpHeader::CWR | TcpHeader::ECE)) == (TcpHeader::ECE))
        {
                             ...
                             const Address& fromAddress, const Address& toAddress)
{
   ...
   * packet and the traffic is ECN Capable
   */
  if (m_useEcn && (h.GetFlags () & (TcpHeader::CWR | TcpHeader::ECE)) == (TcpHeader::CWR | TcpHeader::ECE))
    {
      SendEmptyPacket (TcpHeader::SYN | TcpHeader::ACK | TcpHeader::ECE);
      NS_LOG_DEBUG (TcpSocketState::EcnStateName[m_tcb->m_ecnState] << " -> ECN_IDLE");
      m_tcb->m_ecnState = TcpSocketState::ECN_IDLE;
...
TcpSocketBase::AdvertisedWindowSize (bool scale) const
{
  ...
  if (m_tcb->m_rxBuffer->GotFin ())
    ...
    {
      uint32_t max = m_tcb->m_rxBuffer->MaxRxSequence ().GetValue ();
      uint32_t next = m_tcb->m_rxBuffer->NextRxSequence ().GetValue ();
